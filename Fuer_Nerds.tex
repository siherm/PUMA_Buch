...
Es werden nur die Einträge angezeigt, die von Benutzern stammen, auf deren Freundesliste man steht und die von diesen Benutzern für Freunde freigegeben wurden (für Freunde sichtbar).
...
\begin{description}
    ...
    Zeigt alle eigenen Einträge an, die als \enquote{öffentlich sichtbar\index{Sichtbarkeit!öffentlich}} eingestellt wurden.
    ...
    Zeigt alle eigenen Einträge an, die als \enquote{privat sichtbar\index{Sichtbarkeit!privat}} eingestellt wurden.
    ...
    Zeigt alle eigenen Einträge an, die als \enquote{für Freunde sichtbar\index{Sichtbarkeit!Freunde}} eingestellt wurden.
...
Immer wenn in PUMA eine Liste von Lesezeichen oder Publikationen ausgegeben wird, können diese sortiert werden, indem an die URL eine oder mehrere der folgenden Suchparameter angehängt werden. Folgende Parameter stehen zur Verfügung:
\begin{description}
...
\todo[inline]{Merke:intrahash: zur Duplikatserkennung innerhalb der Sammlung eines Benutzers) und den interhash (zur Duplikatserkennung zwischen den Sammlungen verschiedener Benutzer)TIP: Wenn man eine Dublette anlegen möchte, z.B. Vortrag an mehreren Orten, muss einer der Felder des Intrahashs belegt werden}
...
Mit Hilfe eines kurzen JavaScript\index{JavaScript}-Codes\footnote{zu finden unter \url{https://ub.uni-stuttgart.de/buttons}} kann ein Link auf die eigene Seite eingefügt werden, mit dem die Besucher der Seite mit einem Klick diese in ihre eigene PUMA-Sammlung übernehmen können.
...
Um die Inhalte des JSON-Feeds auf eigenen Seite anzuzeigen, kann beispielsweise die JavaScript-Bibliothek Exhibit \footnote{\url{http://www.simile-widgets.org/exhibit/}} verwendet werden. Der folgende Code stellt eine Publikationsliste aus PUMA als JSON-Feed für Exhibit zur Verfügung.
...
\todo[inline]{brauchen wir das hier noch? Ist bestimmt veraltet}
\section{Plugins} 
\label{sec:plugins}
...
\todo[inline]{stimmt so nicht mehr}
