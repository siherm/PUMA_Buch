...
Wenn Sie einen Tag verwenden möchten, der aus mehreren Worten besteht (z.B. Fachbereich Architektur) dann verwenden Sie PascalCase\index{PascalCase} (z.B. FachbereichArchitektur). 
\end{mdframed}
%\end{wrapfigure}
...
\newline
Systemtags\index{Tags!Systemtags} sind spezielle Tags, die eine feste Bedeutung haben. Derzeit bietet PUMA drei Typen von Systemtags an: \newline\newline
...
\begin{mdframed}[style=mdfexample1,frametitle={ACHTUNG},backgroundcolor=gray!40]Hier kann ebenfalls immer nur ein Tag in das Feld Subtag eingegeben werden, da sonst die Aktion nicht durchgeführt wird.
\end{mdframed}
...
\begin{mdframed}[style=mdfexample1,frametitle={BEDINGUNG},backgroundcolor=gray!40]Um an eine Publikation eine Datei anzuhängen, muss die Publikation in Ihrer Sammlung eingetragen sein.\end{mdframed}
\begin{enumerate}
...
\end{enumerate}
\subsubsection{Eigene Einträge bearbeiten}
